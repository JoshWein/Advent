 ...
 * Advent6.java
 *
 * Created on Dec 6, 2015, 2:03:11 PM
 *
 *
 */
...
public class Advent6 {
    public static void main(String [] args) {
        ...
        int[][] grid = new int[1000][1000]; // Set up grid
        int fromRow, fromCol, toRow, toCol;
        int change;
        long a = System.currentTimeMillis(); // timer
        for(String inst : input) { // get each line
            String[] command = inst.split(" ");
            switch(command[0]) {
                case "turn":
                    fromRow = Integer.parseInt(command[2].split(",")[0]);
                    fromCol = Integer.parseInt(command[2].split(",")[1]);
                    toRow = Integer.parseInt(command[4].split(",")[0]);
                    toCol = Integer.parseInt(command[4].split(",")[1]);
                    change = -1;
                    switch(command[1]) {
                        case "on":
                            change = 1;
                    ...
                    break;
                default: // case toggle
                    fromRow = Integer.parseInt(command[1].split(",")[0]);
                    fromCol = Integer.parseInt(command[1].split(",")[1]);
                    toRow = Integer.parseInt(command[3].split(",")[0]);
                    toCol = Integer.parseInt(command[3].split(",")[1]);
                    for(int row = fromRow; row <= toRow; row++)
                        for(int col = fromCol; col <= toCol; col++)
                            grid[row][col] += 2; // flip variable
                    break;
            }
        }
    ...
    static String getInput() {
        return "turn off 660,55 through 986,197\n" +
